"""Create an editable main.tex and refresh its generated data and assets."""
import logging
...
def _assemble(output_dir, report, template):
    ...
    editable_body = body.replace('% Machine generated by tuttireporting. Do not edit.',
                                 '% Report sections: edit text and add commentary below.', 1)
    existing_main = output / 'main.tex'
    legacy_layout = (existing_main.exists()
                     and r'\input{generated_body.tex}' in existing_main.read_text(encoding='utf-8'))
    for comment in comments:
        ...
        if comment['style'] == 'block':
            ...
            content += prompt + '\n\n\n\n\n\n'
        editable_body = editable_body.replace(
            '% Reviewer commentary: edit the preserved file below.\n'
            + r'\input{' + comment['path'] + '}',
            '\n' + (target.read_text(encoding='utf-8') if target.exists() else content) + '\n')
    ...
    _write(output / 'generated_body.tex', body if legacy_layout else editable_body)
    main = main.replace(r'\input{generated_body.tex}', editable_body)
