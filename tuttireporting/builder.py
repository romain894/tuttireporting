...
def _assemble(output_dir, report, template):
    ...
    comments = [section['reviewer_comment'] for section in report.sections
                if section.get('reviewer_comment')]
    ...
    for path in [output / 'plots', output / 'tutti', output / 'comments', output / 'main.tex',
                 output / 'generated_variables.tex', output / 'generated_body.tex',
                 output / 'generated_bibliography.tex', output / '.tutti-template.json']:
        if path.is_symlink():
    ...
    for _, relative in copies:
        if Path(relative).parts[0] == 'comments':
            raise ValueError('The comments directory is reserved for reviewer text')
    for relative in [*(relative for _, relative in copies),
                     *(comment['path'] for comment in comments)]:
    ...
    for comment in comments:
        target = output / comment['path']
        target.parent.mkdir(parents=True, exist_ok=True)
        prompt = '\n'.join('% ' + line for line in comment['prompt'].splitlines())
        content = '% Human-owned: preserved when rebuilding. Write LaTeX below.\n'
        ...
        try:
            ...
            log.info('Preserving reviewer commentary: %s', target)
    _write(output / 'generated_variables.tex', variables)
    _write(output / 'generated_body.tex', body)
    enabled = (report.metadata['include_bibliography']
               and 'tuttiBibliographyFile' in report.variables)
    _write(output / 'generated_bibliography.tex',
           '% Regenerated by tuttireporting; uncomment to include the bibliography.\n'
           + ('' if enabled else '% ') + '\\makebiblio\n')
